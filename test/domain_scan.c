...
char *get_path(int server_id, char *domain_name, char *suffix)
{
	...
	if (!stricmp(suffix, "cn"))
	{
		...
		{
			case 0:
				sprintf(path, "cgi-bin1/domainregister/chinreg8.cgi", domain_name, domain_name);
				break;
			case 1:
...
int scan(int server_id, char* filename)
{
    int sockfd;
	FILE *fp;
	int i;
...
int ds_main(int argc, char * argv[])
{
	int i;
	int server_id = 0;
	char filename[128] = "domain_list.txt";
	...
	for (i = 1; i < argc; i++)
	{
		if (!stricmp(argv[i], "--server") || !stricmp(argv[i], "-S"))
		{
			if (argc > i+1)
	...
	for (i = 1; i < argc; i++)
	{
		if (!stricmp(argv[i], "--file") || !stricmp(argv[i], "-F"))
		{
			if (argc > i+1)
	...
	return scan(server_id, filename);
}
