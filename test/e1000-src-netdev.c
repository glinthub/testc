...
static void e1000e_vlan_strip_disable(struct e1000_adapter *adapter)
{
    ...
    printk(KERN_WARNING "zws e1000e_vlan_strip_disable\n");
	/* disable VLAN tag insert/strip */
	ctrl = er32(CTRL);
	ctrl &= ~E1000_CTRL_VME;
	ew32(CTRL, ctrl);
...
#ifdef ZWS
        int err;
        err = pskb_expand_head(skb, 2, 0, GFP_ATOMIC);
        if (err)
            printk(KERN_WARNING "zws: pskb_expand_head error %d\n", err);
        skb_push(skb, 4);
        memcpy(skb->data, skb->data+4, 12);
        *(unsigned short *)(skb->data+12) = htons(0x8100);
        *(unsigned short *)(skb->data+14) = htons(vlan_tx_tag_get(skb));
 ...
 **/
void e1000e_vlan_strip_disable(struct e1000_adapter *adapter);
#ifdef HAVE_CONFIG_HOTPLUG
